...
def submit_feedback(request):
    if request.method == 'POST':
        ...
        # Oxirgi arizani vaqtini tekshiramiz
        ...
        if last_app:
            ...
            if time_passed < timedelta(seconds=60):
                ...
                # Cooldown ekrani (Sening uslubingda, lekin ogohlantirish bilan)
                return HttpResponse(f'''
                    <div class="flex flex-col items-center justify-center h-full space-y-6 p-8 animate-in fade-in zoom-in duration-500">
                        <div class="w-24 h-24 bg-yellow-500/10 border-2 border-yellow-500 rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(234,179,8,0.2)]">
                            <svg class="w-12 h-12 text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="Wait M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                        </div>
        ...
        # 2. Telegram xabari uchun lug'at
        category_config = {
            'news': ('📢', 'YANGILIK'),
            'ads': ('💰', 'REKLAMA'),
            'report': ('⚠️', 'SHIKOYAT'),
